...
- [x] Add run routes, synthetic live fixture coverage, screenshot storyboard proof, and operational guidance.

| Date       | Decision                                                                                                                        | Reason                                                                                                                       |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| 2026-09-09 | Send only run identity and an optional event sequence on private Realtime topics, then fetch canonical events and lifecycle state. | Broadcast wakes the client without making an ephemeral transport the source of truth or duplicating event content.           |
...
| 2026-09-09 | Associate content-addressed screenshots with every capturing run and sign only private PNG/JPEG records for at most five minutes.  | Deduplication must retain each run's storyboard without allowing guessed IDs or public buckets to bypass expiring access.     |
| 2026-09-09 | Allocate a distinct immutable resume decision for every pause and start normally when a pre-execution pause has no checkpoint.     | Repeated pauses preserve response history, while a queued run has no graph state to continue.                                |
