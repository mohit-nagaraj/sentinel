...
- Explorer runtime and ports: `packages/orchestration/src/application-explorer.ts`; shared-kernel composition: `packages/orchestration/src/application-explorer-specialist.ts`. Both are exported by `@sentinel/orchestration`.
...
- The shared composition fixes the Application identity, all four modes, versioned prompt/model/toolset/completion IDs, and four described strict tools. Rich browser/frontier/replay state remains in an injected durable store while SNT-014 owns checkpoint, lease, budget, event, and authorized-resume behavior.
- Recovery revalidates the checkpointed authentication reference and page fingerprint before replay. Complete results require observed transition evidence addressing every question and success criterion, and all non-interrupted terminal paths close the browser session.
