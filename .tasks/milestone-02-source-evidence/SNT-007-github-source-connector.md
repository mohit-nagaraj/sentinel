...
- Metadata: `GitHubMetadataClient` uses `@octokit/rest` for repository, commit, PR, and comparison requests. It accepts anonymous access or an in-memory token and returns normalized identities plus full immutable SHAs.
- Checkout: `EphemeralCheckoutManager` creates a random lease under the OS temporary directory, initializes a bare object store, runs `git fetch --no-tags --no-recurse-submodules --depth=1 <remote> <sha>:<private-ref>`, validates the tree, and creates detached worktrees for the requested source/base/head snapshots.
- Git runs without a shell, prompts, credential helpers, system/global Git configuration, submodule recursion, or line-ending conversion. Token authentication uses a process-local, host-scoped extra header and never embeds credentials in a remote URL or error object.
...
- Source reads require a validated relative path, reject symbolic-link components, verify realpath containment and the Git-recorded size, open with `O_NOFOLLOW`, and reject NUL-containing or invalid UTF-8 content.
- `startGitHubSourceConnector` reclaims expired Sentinel lease directories at startup. `withCheckout` cleans after success or callback failure; checkout construction cleans on Git failure, timeout, or cancellation; explicit handles expose idempotent `dispose`.
...
- Local integration fixture: `tests/fixtures/git-repository.ts` creates a bare repository with base/head branches, rename/deletion, binary content, an escaping symlink object, and a gitlink without executing repository code.
- Verification on 2026-09-07: `pnpm test` passed 184 tests; `pnpm test:integration` passed 8 Git tests with 10 credential-gated Supabase tests skipped; `pnpm typecheck`, `pnpm lint`, `pnpm format:check`, and `pnpm build` passed.
