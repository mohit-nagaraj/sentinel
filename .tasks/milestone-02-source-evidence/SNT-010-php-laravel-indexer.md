...
- Code Explorer preparation: `PhpCodeIndex` provides bounded symbol search, smallest-enclosing-declaration lookup, relationship neighborhoods capped by depth/symbol/relationship counts, and commit/content-bound source slices capped by context lines, total lines, and characters. Multi-declaration symbols require the exact occurrence match and never fall back to an ambiguous envelope slice.
