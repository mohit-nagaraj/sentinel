...
- Fixture coverage: `tests/fixtures/php-laravel` traces grouped route → Action → Handler → Service → Repository → model and covers aliases, attributes, traits, interfaces, promoted constructor properties, closure/arrow shadowing, FormRequest, JsonResource, UTF-8 BOM normalization, malformed source, inert top-level code, unrelated Route classes, computed route components, and dynamic negatives. Golden expectations are consumed through the TypeScript schema.
- Verification on 2026-09-07: Composer strict validation and PHP syntax checks passed; `pnpm test` passed 241 tests; `pnpm test:integration` passed 24 tests with 11 environment-gated Supabase/LangGraph tests skipped; `pnpm typecheck`, `pnpm lint`, `pnpm format:check`, and `pnpm build` passed.
